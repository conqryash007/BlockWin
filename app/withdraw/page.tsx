...
export default function WithdrawPage() {
  ...
  const handleExecuteWithdraw = async () => {
    if (!network) return;
    const result = (await executeWithdraw(network)) as {
      success: boolean;
      txHash?: string;
      error?: string;
    };
  ...
  const allowanceBig = allowanceData
              ...
              <CardContent className="space-y-6 text-center py-8">
                ...
                <div>
                  <h3 className="text-lg font-medium text-white mb-2">
                    Connect your {network === "ethereum" ? "Ethereum" : "TRON"}{" "}
                    wallet
                  </h3>
                  <p className="text-muted-foreground max-w-xs mx-auto">
                    You need to connect a wallet on the supported network to
                    receive your funds.
                  </p>
                </div>
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                        ...
                        <p className="text-lg font-bold text-white font-mono break-all line-clamp-1">
                          {formatUsdt(
                            Number(allowanceData.contractBalance) / 1e6,
                          )}
                        </p>
                      </div>
                    </div>
                    ...
                    {(() => {
                      const actualWithdrawAmount =
                        Number(allowanceData.allowance) / 1e6;
                      ...
                      return (
                        <div className="p-6 rounded-xl bg-gradient-to-br from-white/5 to-transparent border border-white/5 space-y-3">
                          <div className="flex justify-between">
                            ...
                            <span className="text-white font-medium">
                              {formatUsdt(actualWithdrawAmount)}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">Fee</span>
                            <span className="text-white font-medium">
                              {formatUsdt(actualFee)}
                            </span>
                          </div>
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-casino-brand font-bold text-xl">
                              {formatUsdt(actualReceive)}
                            </span>
                          </div>
                        </div>
                      );
