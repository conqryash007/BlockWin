...
export function useWithdrawal() {
  ...
  const isWithdrawConfirming = withdrawHash
    ? isConfirmLoading || isConfirmFetching
    : false;
  ...
  const checkAllowance = useCallback(
    async (
    ...
    ): Promise<WithdrawalAllowanceResult | null> => {
      ...
      try {
        ...
        const res = await fetch(`/api/withdrawal/allowance?${params}`, {
          headers,
        });
        if (!res.ok) {
  ...
  const executeWithdraw = useCallback(
    async (
      ...
      if (network === "ethereum") {
        if (!evmAddress)
          return { success: false, error: "Connect your Ethereum wallet" };
