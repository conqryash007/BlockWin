...
export class BallManager {
    ...
    constructor(canvasRef: HTMLCanvasElement, onFinish?: (index: number,startX?: number) => void, onCollision?: () => void) {
        this.balls = [];
    ...
    drawSinks() {
        ...
        for (let i = 0; i<this.sinks.length; i++)  {
            ...
            this.ctx.fillText((sink?.multiplier)?.toString() + "x", sink.x - 10 + sinkWidth / 2, sink.y);
        };
    }
