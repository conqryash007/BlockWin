...
export class Ball {
    ...
    constructor(x: number, y: number, radius: number, color: string, ctx: CanvasRenderingContext2D, obstacles: Obstacle[], sinks: Sink[], onFinish: (index: number) => void, onCollision?: () => void) {
      this.x = x;
      this.y = y;
      this.radius = radius;
      this.color = color;
