...
export function MinesGamePage() {
  ...
  const calculateMultiplier = useCallback(
    (revealed: number, mines: number): number => {
  ...
  const handleTileClick = async (tileId: number) => {
    ...
    try {
      ...
      const revealedTileIndex =
        data.revealedTile !== undefined ? data.revealedTile : tileId;
      ...
      if (data.hitMine) {
        ...
        toast.error(`💥 BOOM! You hit a mine. -$${betAmount.toFixed(2)}`);
      } else {
        // Safe - reveal the tile returned by backend
        newGrid[revealedTileIndex] = {
          ...newGrid[revealedTileIndex],
          state: "safe",
        };
        setGrid(newGrid);
  ...
  const potentialPayout =
    betAmount * calculateMultiplier(revealedCount, actualMineCount);
