...
export function CrashBetControls({
  ...
  const canPlaceBet = gameState === "waiting" && betAmount > 0 && betAmount <= balance && !hasBet && !isProcessing;
  const canCashOut = gameState === "running" && hasBet && !isProcessing;
    ...
    <div className="flex h-full flex-col gap-6 bg-black/20 backdrop-blur-md p-4 md:w-[320px] md:border-r border-white/5 relative z-10">
      <Tabs defaultValue="manual" className="w-full">
        <TabsList className="grid w-full grid-cols-2 bg-black/40 border border-white/5 p-1 h-12">
          ...
          <TabsTrigger value="auto" className="data-[state=active]:bg-casino-brand data-[state=active]:text-black font-bold h-full" disabled>Auto</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="manual" className="space-y-5 mt-6">
          ...
          <div className="space-y-3">
            ...
            <div className="relative group">
              ...
              <Input 
                ...
                disabled={gameState !== "waiting" || hasBet || isProcessing}
          ...
          <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
            <p className="text-xs text-blue-400">
              <span className="font-bold">Max Multiplier:</span> 5.00x | Cash out before the crash!
            </p>
          </div>
          ...
          {gameState === "waiting" && !hasBet && (
            ...
            >
              {isProcessing ? "Starting..." : "Place Bet"}
            </Button>
          )}

          {/* Starting state - show disabled Cash Out button */}
          {gameState === "starting" && (
            <Button 
              disabled={true}
              className="w-full h-20 text-xl font-black uppercase tracking-widest bg-gray-500 text-white border-0 mt-2 rounded-xl opacity-50 cursor-not-allowed"
            >
              <div className="flex flex-col items-center">
                <span>Cash Out</span>
                <span className="text-sm text-gray-300">Starting game...</span>
              </div>
            </Button>
          )}
          ...
          {gameState === "running" && hasBet && (
            <Button 
              onClick={onCashOut}
              disabled={!canCashOut}
              className="w-full h-20 text-xl font-black uppercase tracking-widest bg-green-500 text-white hover:bg-green-400 shadow-[0_0_30px_rgba(34,197,94,0.5)] transition-all hover:scale-[1.02] active:scale-[0.98] border-0 mt-2 rounded-xl animate-pulse disabled:opacity-50"
              style={{ animationDuration: '1s' }}
            >
              <div className="flex flex-col items-center">
                <span>{isProcessing ? "Cashing Out..." : "Cash Out"}</span>
          ...
          {gameState === "crashed" && (
            <div className="w-full h-16 flex items-center justify-center bg-red-500/20 border-2 border-red-500/30 rounded-xl animate-in fade-in">
              <span className="text-lg font-bold text-red-400">
                {isProcessing ? "Updating balance..." : "Round ended"}
              </span>
            </div>
          )}
        </TabsContent>
